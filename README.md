...
> ⚠️ **强烈注意：从 V1 升级到 V2 的用户**
> 如果您之前安装过旧版本，**必须重新下载并覆盖**服务端的 `plugins/autopulse/index.js` 文件，然后**重启酒馆后台**！
> 否则情绪压力系统（动态缩短时间）将无法生效，因为旧版服务端无法识别新的压力参数！
...
> ⚠️ **这一步必须手动操作**，否则关闭浏览器后定时器会停止，且**情绪压力系统无法工作**。

1. 在 SillyTavern 根目录下找到 `plugins/` 文件夹（没有就新建）
2. 在 `plugins/` 下创建 `autopulse/` 文件夹
3. 将本仓库中的 `server-plugin/index.js` 复制为 `plugins/autopulse/index.js`（**升级用户请直接覆盖旧文件**）
