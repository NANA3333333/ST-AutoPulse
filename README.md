...
- 修复离线队列重复补发的问题，已成功处理的事件现在会按 `eventId` 确认并从队列移除。
- 修复定时消息和定时任务可能串到错误角色聊天的问题，服务端事件现在会携带 `characterId`，前端只在当前角色匹配时处理。
- 修复嫉妒系统在切换角色后丢失原角色上下文的问题，现已加入上下文缓存，生成结果更贴近离开前的聊天内容。
- 修复压力系统本地状态与服务端计时器状态不同步的问题，切换聊天、恢复页面后状态会重新对齐。
- 修复独立 API 配置保存时缺少请求头导致的保存失败问题。
- 优化独立 API Endpoint 兼容性，填写站点根地址时会自动补全到常见的 OpenAI 兼容接口路径。
- 修复 `Test Jealousy` 按钮错误读取旧聊天上下文的问题，现在会实时读取当前打开的角色聊天。
